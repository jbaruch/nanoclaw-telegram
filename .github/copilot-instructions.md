...
6. Check for existing PRs/issues: `gh pr list --repo jbaruch/nanoclaw --search "<topic>"`. Never target `qwibitai/*` — the upstream fork is off-limits without explicit permission (per the `nanoclaw-host: repo-chain` rule; see `groups/global/BASH_SAFETY.md` § PR creation).
