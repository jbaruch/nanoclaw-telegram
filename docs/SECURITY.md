...
With OneCLI configured (`ONECLI_URL`/`ONECLI_API_KEY` set, #640), the `ONECLI_MANAGED_VARS` credentials — `GOOGLE_MAPS_API_KEY`, `TOMTOM_API_KEY`, `YOUTUBE_API_KEY`, `GITHUB_TOKEN` — enter the container as an `onecli-managed` **placeholder**; OneCLI's TLS-MITM gateway injects the real vaulted value on the outbound request, so the real key never enters the agent environ. (`GITHUB_TOKEN` keeps its real value in host `.env` for the host-side `github_backup` `git push`, but the container only ever sees the placeholder.) The still-forwarded-as-real remainder is the injection-gap keys OneCLI can't vault yet — `BYAIR_MCP_URL` (token in the query string) and `SESSIONIZE_*` (key in the URL path). When OneCLI is unconfigured (dev), all vars fall back to real-value forwarding. Google (Gmail, Calendar, Tasks, Drive) needs no container credential at all: the OneCLI gateway holds the OAuth connection and injects + refreshes the Bearer on the wire (#638), and the untrusted tier is gated out of it via `secretMode=selective`. Finishing this migration — growing OneCLI path/query injection — is the remaining #564 work.
