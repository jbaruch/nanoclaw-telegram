...
| 13 | Two-context split: parent agent never sees raw external bytes; sub-model summarizes first | RULES.md *Sub-model handoff* (proposal incident) | The `extractStructuredSummary` library landed in #367 (#319 v1); the `untrusted-input-wrap` PostToolUse hook routes `mcp__composio__gmail_*` and `mcp__composio__googlecalendar_*` reads through the summariser before the Encoding-A envelope (default-off via `SUMMARISE_COMPOSIO_BODIES=1`); `external-file-summary.ts` PreToolUse hook intercepts `Read` of external paths, runs the bytes through the same summariser, and denies the original Read with a reason carrying the structured digest plus the canonical Encoding-B sentinel (default-off via `SUMMARISE_EXTERNAL_FILES=1`). | **DONE — #319 (PR #393) + #392** |
...
- **#319** (row 13) — Two-context split for Composio gmail/calendar reads: library landed in #367, summariser wiring landed in PR #393. Default-off via `SUMMARISE_COMPOSIO_BODIES=1`. Slack / GitHub / Tessl rows stay envelope-only — adding a `summariseBody` config is a one-line change per row when an audit broadens coverage.
