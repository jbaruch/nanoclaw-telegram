...
| 13 | Two-context split: parent agent never sees raw external bytes; sub-model summarizes first | RULES.md *Sub-model handoff* (proposal incident) | The `extractStructuredSummary` library landed in #367 (#319 v1); the `untrusted-input-wrap` PostToolUse hook now routes `mcp__composio__gmail_*` and `mcp__composio__googlecalendar_*` reads through the summariser before the Encoding-A envelope (default-off via `SUMMARISE_COMPOSIO_BODIES=1`). `Read` of external file paths is the remaining open piece, tracked separately on #392. | **DONE for Composio gmail/calendar — #319; OPEN for Read external paths — #392** |
...
- **#319** (row 13) — Two-context split for Composio gmail/calendar reads: library landed in #367, summariser wiring landed in this PR. Default-off via `SUMMARISE_COMPOSIO_BODIES=1`. Slack / GitHub / Tessl rows stay envelope-only — adding a `summariseBody` config is a one-line change per row when an audit broadens coverage.
- **#392** (row 13, secondary) — Two-context split for `Read` of external file paths: separate code path (PostToolUse `Read` matcher in `index.ts`); ships independently.
