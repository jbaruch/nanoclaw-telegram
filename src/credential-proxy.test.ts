...
describe('credential-proxy', () => {
  ...
  // #640: the gated auth-path trace (CREDPROXY_AUTH_DEBUG=1). These verify the
  // observable payload — that it is emitted only when the flag is on, uses an
  // allowlisted endpoint label (never raw path material, per no-secrets), and
  // that oneCliUsed reflects what was actually applied.
  function authDebugPayloads(): Array<Record<string, unknown>> {
    return vi
      .mocked(logger.info)
      .mock.calls.filter((c) => c[1] === 'credproxy-auth-debug')
      .map((c) => c[0] as Record<string, unknown>);
  }

  it('CREDPROXY_AUTH_DEBUG off: emits no auth-path trace', async () => {
    ...
    expect(authDebugPayloads()).toHaveLength(0);
  });

  it('CREDPROXY_AUTH_DEBUG on: logs an allowlisted endpoint label and never raw path material', async () => {
    process.env.CREDPROXY_AUTH_DEBUG = '1';
    vi.mocked(logger.info).mockClear();
    try {
      proxyPort = await startProxy({
        CLAUDE_CODE_OAUTH_TOKEN: 'real-oauth-token',
      ...
      // A path carrying a secret-looking segment must classify to the fixed
      // 'other' label — the raw segment must never reach the log.
      await makeRequest(
        proxyPort,
        {
          method: 'POST',
          path: '/secret/sk-ant-should-not-log',
          headers: {
            'content-type': 'application/json',
            authorization: 'Bearer placeholder',
          },
      ...
      const payloads = authDebugPayloads();
      expect(payloads).toHaveLength(3);

      const [other, messages, oauth] = payloads;
      ...
      expect(oauth).toMatchObject({ endpoint: 'oauth/create_api_key' });

      // No payload carries a raw-path field or the secret path segment.
      for (const p of payloads) {
        expect(p).not.toHaveProperty('pathSuffix');
        expect(JSON.stringify(p)).not.toContain('sk-ant-should-not-log');
      }
    } finally {
      delete process.env.CREDPROXY_AUTH_DEBUG;
    }
  });

  it('CREDPROXY_AUTH_DEBUG on: oneCliUsed reflects actual application (OneCLI + HTTPS upstream)', async () => {
    process.env.CREDPROXY_AUTH_DEBUG = '1';
    vi.mocked(logger.info).mockClear();
    vi.mocked(isOneCliConfigured).mockReturnValue(true);
    vi.mocked(getOneCliOutboundConfig).mockResolvedValue({
    ...
    try {
      ...
      const [payload] = authDebugPayloads();
