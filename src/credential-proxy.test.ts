...
describe('credential-proxy', () => {
  ...
  // ANTHROPIC_BASE_URL from .env is the custom-endpoint override the
  // README documents for third-party Anthropic-compatible models
  // (Ollama proxy, Together, Fireworks). It survived the LiteLLM
  // decommission — only the gateway-cutover role is gone.
  it('routes to the ANTHROPIC_BASE_URL env upstream when no test override is set', async () => {
  ...
  it('warns and degrades to anthropic-direct on a malformed ANTHROPIC_BASE_URL', async () => {
    Object.assign(mockEnv, {
      ANTHROPIC_API_KEY: 'sk-ant-real-key',
      ANTHROPIC_BASE_URL: 'not a url ###',
    });
    // Startup must not throw; the malformed value degrades with a warn.
    proxyServer = await startCredentialProxy(0, '127.0.0.1');
    const { logger } = await import('./logger.js');
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining('ANTHROPIC_BASE_URL is not a valid URL'),
    );
  });

  it('test upstreamUrl override takes precedence over the env upstream', async () => {
    // Env points at a dead port; the test override points at the live
    // mock. The request landing proves the override wins — the seam
