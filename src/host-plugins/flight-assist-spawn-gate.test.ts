...
// The gate registers through the same host-plugin entry point production
// startup uses (#846) — the registration is idempotent, so calling it in
// every test file that needs the gate is safe.
registerFlightAssistSpawnGate();

describe('flight-assist trip-window gate', () => {
  let groupDir: string;
