...
/**
 * Flight-assist trip-window spawn gate (#754), extracted from core
 * `spawn-gates.ts` as a host-plugin module (#846). The MECHANISM (gate
 * registry, resolver, fail-open/closed errno narrowing) stays in core;
 * this module owns the POLICY — the `tessl__flight-assist` skill is
 * only worth spawning inside a trip window derived from the group's
 * `travel-db.json`.
 */
...
/**
 * Register the trip-window gate for the flight-assist skill. Idempotent
 * so tests and startup can both call it without a duplicate-registration
 * throw. `Skill(skill: "tessl__flight-assist")` → `parseTaskSkill`
 * returns the key registered here.
 */
export function registerFlightAssistSpawnGate(): void {
  if (registered) return;
  registered = true;
  registerSpawnGate('tessl__flight-assist', tripWindowGate);
}
