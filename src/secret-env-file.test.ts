...
describe('SECRET_CONTAINER_VARS', () => {
  it('no longer lists any COMPOSIO_* var (retired with the Composio MCP surface, jbaruch/nanoclaw#639)', () => {
    for (const name of SECRET_CONTAINER_VARS) {
      expect(name.startsWith('COMPOSIO_')).toBe(false);
    }
  });

  it('lists GITHUB_TOKEN (`gh` CLI inside main/trusted containers — OneCLI migration target jbaruch/nanoclaw#564)', () => {
...
describe('buildSecretEnvFile', () => {
  ...
  it('skips empty-string values (treats as "not set")', () => {
    expect(buildSecretEnvFile({ GITHUB_TOKEN: '' })).toBeNull();
  });

  it('writes the env-file with mode 0600 and emits --env-file args', () => {
    const result = buildSecretEnvFile({ GITHUB_TOKEN: 'sk-real-secret-1' });
    ...
    expect(content).toBe('GITHUB_TOKEN=sk-real-secret-1\n');
  });
  ...
  it('cleanup() removes the env-file', () => {
    const result = buildSecretEnvFile({ GITHUB_TOKEN: 'sk-cleanup-test' });
    expect(result).not.toBeNull();
    const filePath = result!.args[1];
  ...
  it('cleanup() is idempotent — safe to call from both close and error handlers', () => {
    const result = buildSecretEnvFile({ GITHUB_TOKEN: 'sk-idem' });
    expect(result).not.toBeNull();
    const filePath = result!.args[1];
  ...
  it('uses the random suffix from crypto.randomBytes (deterministic via mock)', () => {
    ...
    const result = buildSecretEnvFile({ GITHUB_TOKEN: 'sk-pinned' });
    expect(result).not.toBeNull();
  ...
  it('writes the env-file before returning so docker can read it immediately', () => {
    const result = buildSecretEnvFile({ GITHUB_TOKEN: 'sk-sync' });
    expect(result).not.toBeNull();
    tempFilesToCleanup.push(result!.args[1]);
...
describe('buildSecretEnvFile — write-failure cleanup', () => {
  it('removes the tempfile when the write fails (no orphaned partial-secret file)', () => {
    ...
    expect(() => buildSecretEnvFile({ GITHUB_TOKEN: 'sk-write-fail' })).toThrow(
      /simulated EIO/,
    );
...
describe('buildSecretEnvFile — symlink-race defense', () => {
  it('throws and leaves a pre-existing path untouched (O_EXCL behavior)', () => {
    ...
    expect(() =>
      buildSecretEnvFile({ GITHUB_TOKEN: 'sk-symlink-race' }),
    ).toThrow(/EEXIST/);
