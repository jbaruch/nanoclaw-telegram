...
**Which mute type to use:** Mailing list (groups.io, listserv, etc.) → **subject pattern** (new replies get new threadIds, so threadId muting silently fails). Everything else → **threadId**.

**To mute:** When Baruch says "не интересно", "mute this", "stop reporting this", or any dismissal — determine type, append to the right array in `email-mute-state.json` **immediately, before anything else**. Any session (main, heartbeat, background). No waiting, no asking.

- *From main session:* Use `GMAIL_FETCH_EMAILS` to find the thread by subject/sender. Check for mailing list origin → subject pattern; otherwise → threadId. Confirm with: "✓ muted".
- *Repeated dismissals = bug.* If Baruch dismisses the same thread twice, the mute wasn't saved or the wrong type was used. Check if a threadId mute is failing due to mailing list behavior and convert it to a subject pattern.

## Load Preferences

Read `/workspace/group/email-preferences.json` if it exists. Schema:

- `always_important`: array of `{sender?, domain?, reason}` — skip calibration, always flag
- `always_ignore`: array of `{sender?, domain?, reason}` — skip calibration, always drop
- `patterns`: array of `{rule, action ("important"|"ignore"), reason}` — applied during classification

These override all calibration and classification rules. If the file doesn't exist or is empty, proceed with default rules only.
...
|---|---|
| "bad fit" on a LinkedIn notification | `always_ignore` domain: `linkedin.com` |
...
| "you missed one from my boss" | `always_important` sender + domain if unrecognized |
