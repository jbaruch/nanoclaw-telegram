...
1. Load cursor state from `nanoclaw-state.json`
2. Connect to `messages.db` and detect the current chat
3. Query all unanswered user messages since the last cursor
4. Persist the updated cursor back to the state file
5. Return results; validate with the verification query below

## Script

Save as `/workspace/group/check_unanswered.py` and run with `python check_unanswered.py`.
...
if os.path.exists(STATE_FILE):
    with open(STATE_FILE) as f:
        try:
            ...
            pass  # treat corrupt state as empty
