...
3. For each missed task, **immediately write `last_run_date = today's local date`** for that task into `task-tz-state.json`. Then invoke the skill:
   - `morning-brief` → `Skill(skill: "tessl__morning-brief")`
   - `nightly-housekeeping` → `Skill(skill: "tessl__nightly-housekeeping")`
4. The skill also updates `last_run_date` at the end — but heartbeat sets it first
...
2. **Use judgment to respond based on context:**
   - **Still actionable** → apologize for delay and act on it or respond directly
   - **Trivial/casual** → brief acknowledgement
   - **Too late to act** → acknowledge the miss honestly, don't act
...
**Already reported:** Skip emails already surfaced in a previous heartbeat this session.
...
```
If nothing was reported, skip this step entirely.
