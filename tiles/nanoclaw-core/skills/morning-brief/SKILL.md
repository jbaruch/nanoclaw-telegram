...
Read `current_tz` from `/workspace/group/task-tz-state.json` (Step 0). **All time operations use `current_tz`** — calendar window boundaries, event display times, and reminder scheduling. Fall back to `home_tz` or the `TZ` env var if missing.
...
- Reminder scheduling (Steps 8c/8d): follow the `scheduler-timezone` skill protocol — convert event local time → UTC → scheduler timezone. Store `utc_time` with a `Z` suffix.

Whenever a step says "per the [Timezone Reference](#timezone-reference)", apply the rules above directly — no additional inline restatement is needed.
...
**Timezone:** Extract `current_tz` per the [Timezone Reference](#timezone-reference).

> **Note:** Step 10 performs the final canonical write-back of `last_run_date`; Step 0 is the dedup guard only.

> **Silence rule:** All internal progress notes MUST be wrapped in `<internal>` tags or omitted entirely — they must never appear as plain text output.
...
For each task in `pending.undated_tasks`, attempt to assign a due date automatically by following the procedure in `/workspace/group/skills/UNDATED-TASK-TRIAGE.md`. That document covers: finding the linked email, decoding the full body, determining a due date from explicit deadlines and context clues, falling back to title-only inference, and applying the date via the Tasks update tool.

Only ask the owner if, after that full procedure, no reasonable date can be determined — include that task in "📋 Без даты:" with a specific question. Omit the section entirely if all dates were assigned.
...
### 8b: Deduplicate
Before scheduling a reminder for an event, check if `event_id` already exists in `scheduled-reminders.json`. If found — skip that event.

### 8c: Schedule new reminders
For events not already in `scheduled-reminders.json`, schedule a once-off reminder. Convert event time to UTC per the [Timezone Reference](#timezone-reference), then follow the `scheduler-timezone` skill protocol exactly.
...
Derive `utc_time` from the event's local start time using `current_tz` per the [Timezone Reference](#timezone-reference). Write the updated file back to `/workspace/group/scheduled-reminders.json`.
